 ...
 *
 * so the full paths become /admin/dealers/:dealerCode/integration, etc.
 * This does NOT collide with the existing DELETE /admin/dealers/:dealerCode
 * route (that lives in a different router file) — same prefix, different
 * method+path combos.
 */
...
// Retry sync — pulls the lead from OUR OWN `leads` table (already
// synced from Zoho) rather than a per-record Zoho GET, since
// zohoCrmService.js only exposes bulk fetchOemLeads(), not a
// single-record fetch.
