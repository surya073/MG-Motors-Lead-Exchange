/**
 * mapAppDataToDashboard.js
 * -----------------------------------------------------------------------
 * Converts adminDashboardService.getDashboardSummary()'s response
 * (fetched via services/aiService.js -> fetchAppDataSummary) into the
 * { kpis, charts, activity, table } shape DashboardView renders. Purely
...
export function mapAppDataToDashboard(summary) {
  ...
  const flat = (n) => [1, 1, 1, 1, 1, 1, 1].map(() => n || 1); // placeholder sparkline until per-day history exists
